"""Direct browser check for 데이터분석 blank-state UX.

Scenario:
1) Open 분석 > 차트분석 > 데이터분석
2) Clear ticker input
3) Ensure explicit 안내문 appears (not blank screen)
...
def main() -> int:
    ...
    username = f"ui_blank_{int(time.time())}"
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            ticker_input = sidebar.get_by_role("textbox", name="종목 티커").first
            expect(ticker_input).to_be_visible(timeout=20_000)
            ticker_input.fill("")
            sidebar.get_by_role("button", name="조회").first.click(timeout=20_000)

            expect(page.get_by_text("종목 코드를 입력하세요.")).to_be_visible(timeout=20_000)

            ctx.close()
            browser.close()

        print("[PASS] 데이터분석 빈 입력 상태에서 안내문이 표시되어 blank screen이 발생하지 않습니다.")
        return 0
    finally:
        if proc is not None:
